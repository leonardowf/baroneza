...
export class ConcreteExtractTicketsUseCase implements ExtractTicketsUseCase {
    ...
    constructor(commitExtractor: CommitExtractor, jiraTickerParser: JiraTicketParser) {
    ...
    execute(input: ExtractTicketsInput): Observable<ExtractTicketsOutput> {
        ...
        .pipe(
            map((commits) => this.jiraTickerParser.parse(commits)))
        .pipe(
            map(
                (jiraTickerParsedOutput) => jiraTickerParsedOutput.parsedTickets.map(
                    (parsedTicket): TicketIdCommit => ({ ticketId: parsedTicket.ticket, commit: parsedTicket.value})),
