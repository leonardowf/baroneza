...
import {ReactionsReaderInput, ReactionsReader} from '../workers/reactions-reader';
import { flatMap, map, delay } from 'rxjs/operators';

export interface AskConfirmationUseCase {
    execute(input: AskConfirmationUseCaseInput): Observable<AskConfirmationUseCaseOutput>
}
...
export class SlackAskConfirmationUseCase {
    ...
    constructor(messageSender: MessageSender, reactionsReader: ReactionsReader, confirmationReaction: string, secondsTimeout: number) {
