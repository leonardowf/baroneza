...
export class ConcreteUpdateReleaseUseCase implements UpdateReleaseUseCase {
    ...
    execute(input: UpdateReleaseInput): Observable<UpdateReleaseOutput> {
        return this.extractTicketsUseCase.execute({
    ...
    private changelog(ticketsFixVersionData: TicketFixVersionData[], repository: string, version: string): Observable<CreateChangelogOutput | undefined> {
        const absentFixVersionTickets = ticketsFixVersionData.filter((ticket) => ticket.fixVersionStatus === "ABSENT")
    ...
    private categorizeTicketIds(ticketIdCommits: TicketIdCommit[]): Observable<TicketFixVersionData[]> {
        const streams = ticketIdCommits.map((ticketIdCommit) => {
            return this.jiraService.hasFixVersion(ticketIdCommit.ticketId).pipe(
                map((hasFixVersion): TicketFixVersionData => {
                    return hasFixVersion ? { ...ticketIdCommit, fixVersionStatus: "PRESENT" } : { ...ticketIdCommit, fixVersionStatus: "ABSENT" }
                }),
                catchError((): Observable<TicketFixVersionData> => of({ ...ticketIdCommit, fixVersionStatus: "UNKNOWN" }))
            )
        })
    ...
    private notifyChangelog(createChangelogOutput: CreateChangelogOutput | undefined, channel: string): Observable<void> {
        if (createChangelogOutput) {
            return this.slackMessageSender.send({
                destination: channel,
    ...
    private updateJiraRelease(fromVersion: string, toVersion: string, jiraSuffix: string, project: string): Observable<void> {
    ...
    private buildChangelog(pullRequestNumber: number, repository: string, version: string): Observable<CreateChangelogOutput | undefined> {
        return this.createChangelogUseCase.execute({origin: {type: "pullRequestNumber", number: pullRequestNumber}, repository, version })
    }

    private updateDescriptionOfPr(createChangelogOutput: CreateChangelogOutput, pullRequestNumber: number, repository: string): Observable<void> {
        return this.githubService.updateDescription(pullRequestNumber, this.owner, repository, createChangelogOutput.markdown.content)
    }
