...
describe('the milestone creator', () => {
  it('creates a new milestone if none found', (done) => {
    ...
    // when mocks
    when(githubServiceMock.getMilestone(owner, repository, title)).thenReturn(
      of(undefined)
    );
    when(
      githubServiceMock.createMilestone(owner, repository, title)
    ).thenReturn(of(milestoneId));
  ...
  it('creates a new milestone if none found', (done) => {
    ...
    sut.create(title, repository).subscribe({
      next: (result) => {
        verify(
          githubServiceMock.createMilestone(anything(), anything(), anything())
        ).never();
