import { KeepChangelogParser, ConcreteKeepChangelogParser } from "../../src/workers/keep-changelog-parser";

describe("the keep changelog parser", () => {
    var sut: ConcreteKeepChangelogParser
    ...
    it("with changelog section, has output", () => {
        ...
        expect(sut.parse(text)!.added[1]).toEqual("- Support to Jira version creation")
    ...
    it("with changelog section, many lines, many characthers", () => {
        ...
        expect(sut.parse(text)!.security[0]).toEqual("The word 'jaguar' comes from the indigenous word 'yaguar', which means 'he who kills with one leap'.")

        expect(sut.parse(text)!.security[1]).toEqual("Jaguars used to be found from south-west USA, throughout South America to almost the far north in Argentina. Now, they’ve been virtually eliminated from half of their historic range.")
        expect(sut.parse(text)!.security[2]).toEqual("Males can weigh 120kg (that’s almost 19 stone), but the size of jaguars can vary a lot between regions - jaguars in central America can be roughly half the size of jaguars in the Pantanal. They need that bulk behind them to take on big prey, including giant caiman.")
    })
})