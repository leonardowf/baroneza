import { StartTrainEndpoint, StartTrainDependencies } from "../../src/endpoints/start-train-endpoint";
import { mock, instance, when, anything, verify } from "ts-mockito";
import { StartTrainUseCase, StartTrainUseCaseOutput, StartTrainUseCaseInput } from "../../src/use-cases/start-train-use-case";
import { of } from "rxjs";
import { CreateReleaseEndpoint, CreateReleaseEndpointDependencies, CreateReleaseEndpointInput } from "../../src/endpoints/create-release-endpoint";
...
import { TagEndpoint, TagEndpointDependencies, TagEndpointInput } from "../../src/endpoints/tag-endpoint";
...
describe('The start traint endpoint', () => {
    it('maps to the correct use case', (done) => {
        ...
        when(dependencies.startTrainUseCaseMock.execute(anything())).thenReturn(of(new StartTrainUseCaseOutput()))

        const sut = new StartTrainEndpoint(dependencies)

        sut.execute(new StartTrainUseCaseInput("repo", "baseBranch", "targetBranch", "channel")).subscribe({
            complete: () => {
...
describe('The create release endpoint', () => {
    it('maps to the correct use case', (done) => {
        ...
        when(dependencies.createReleaseUseCaseMock.execute(anything())).thenReturn(of(new CreateBranchUseCaseOutput()))

        const sut = new CreateReleaseEndpoint(dependencies)

        sut.execute(new TestCreateReleaseInput()).subscribe({
            complete: () => {
                verify(dependencies.createReleaseUseCaseMock.execute(anything())).once()
                done()
            }
        })
    });
...
describe('The tag endpoint', () => {
    it('maps to the correct use case', (done) => {
        ...
        when(dependencies.tagUseCaseMock.execute(anything())).thenReturn(of(new TagUseCaseOutput([], [])))
...
class TestCreateReleaseDependencies implements CreateReleaseEndpointDependencies {
