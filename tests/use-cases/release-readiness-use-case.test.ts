...
import { ReleaseReadinessUseCase, ReleaseReadinessInput } from '../../src/use-cases/release-readiness-use-case';
...
import { MessageSender, MessageSenderOutput, KnownMessageType } from '../../src/workers/message-sender';
...
describe('The release readiness use case', () => {
  it('posts readiness message when there are open PRs', (done) => {
    ...
    when(unmergedPRsFetcherMock.fetch(input.repository, input.baseBranch)).thenReturn(of(prs));
    when(releaseReadinessBuilderMock.build(prs)).thenReturn(blocks);
    when(messageSenderMock.send(anything())).thenReturn(of(new MessageSenderOutput('ts', 'channel')));
  ...
  it('posts QA blocking message when no open PRs and some tickets are missing QA plans', (done) => {
    ...
    const qaBlocks: KnownBlock[] = [{ type: 'section', text: { type: 'mrkdwn', text: 'missing' } }];

    when(unmergedPRsFetcherMock.fetch(input.repository, input.baseBranch)).thenReturn(of([]));
    when(mergedPRTicketExtractorMock.extract(anything(), anything())).thenReturn(of(['STO-1']));
    when(qaPlanCheckerMock.check(anything(), anything())).thenReturn(of({ missingQAPlan: ['STO-1'] }));
    when(qaPlanMessageBuilderMock.build(anything(), anything())).thenReturn(qaBlocks);
    when(messageSenderMock.send(anything())).thenReturn(of(new MessageSenderOutput('ts', 'channel')));
  ...
  it('posts ready message when no open PRs and all QA plans are present', (done) => {
    ...
    const readyBlocks: Block[] = [{ type: 'header', text: { type: 'plain_text', text: 'ready' } } as Block];

    when(unmergedPRsFetcherMock.fetch(input.repository, input.baseBranch)).thenReturn(of([]));
    when(mergedPRTicketExtractorMock.extract(anything(), anything())).thenReturn(of(['STO-1']));
    when(qaPlanCheckerMock.check(anything(), anything())).thenReturn(of({ missingQAPlan: [] }));
    when(releaseReadinessBuilderMock.buildReadyMessage()).thenReturn(readyBlocks);
    when(messageSenderMock.send(anything())).thenReturn(of(new MessageSenderOutput('ts', 'channel')));
